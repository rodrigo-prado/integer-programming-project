...
//      cout << ewf << endl;
    ...
    // seta valor "infinito" na diagonal principal
    for (int i = 0; i < dimension; i++) {
    for (int j = 0; j < dimension; j++) {
      if (i == j)
        distMatrix[i][j] = INFINITY_COST;
    }
  }
}
...
string Data::getInstanceName(string nameOfInstance) { // Get the name of instance
  string::size_type loc = nameOfInstance.find_last_of(".", nameOfInstance.size());
  string::size_type loc2 = nameOfInstance.find_last_of("/", nameOfInstance.size());

  string instance;

  if (loc != string::npos) {
    instance.append(nameOfInstance, loc2 + 1, loc - loc2 - 1);
  } else {
    instance.append(nameOfInstance, loc2 + 1, instance.size());
  }
  return instance;
}
